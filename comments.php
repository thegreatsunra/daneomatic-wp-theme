...
<?php $sandbox_comment_alt = 0 ?>
				...
				<div id="comments-list" class="oofcomments">
...
<?php if ( get_comment_type() == "comment" ) : ?>
						<li id="comment-<?php comment_ID() ?>" class="<?php sandbox_comment_class() ?>">
							...
							<div class="comment-meta"><?php printf(__('%1$s &#8211; %2$s', 'sandbox'),
										get_comment_date(),
										get_comment_time(),
...
<?php else : ?>
							...
							<div class="form-input"><input id="author" name="author" type="text" class="text" value="<?php echo $comment_author ?>" size="20" maxlength="20" tabindex="3" /></div>

							<div class="form-label"><label for="email"><?php _e('Email', 'sandbox') ?></label> <?php if ($req) _e('<span class="required">*</span>', 'sandbox') ?></div>
							<div class="form-input"><input id="email" name="email" type="text" class="text" value="<?php echo $comment_author_email ?>" size="20" maxlength="50" tabindex="4" /></div>

							<div class="form-label"><label for="url"><?php _e('Website', 'sandbox') ?></label></div>
							<div class="form-input"><input id="url" name="url" type="text" class="text" value="<?php echo $comment_author_url ?>" size="20" maxlength="50" tabindex="5" /></div>
...
<?php endif /* if ( $user_ID ) */ ?>
							...
							<div class="form-submit"><input id="submit" name="submit" type="submit" class="submit" value="<?php _e('Post Comment', 'sandbox') ?>" tabindex="7" accesskey="P" /><input type="hidden" name="comment_post_ID" value="<?php echo $id; ?>" /></div>
